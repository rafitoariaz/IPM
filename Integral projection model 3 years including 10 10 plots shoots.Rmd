...
for (i in 1:2){
  ...
  growth.survival.data[[i]]$size.x<-growth.survival.data[[i]]$size.x
  colnames(growth.survival.data[[i]])[4]<-"size"
}

for (i in 1:2){
  #growth.survival.data[[i]]$size.y<-growth.survival.data[[i]]$size.y+1
  growth.survival.data[[i]]$size.y<-growth.survival.data[[i]]$size.y
...
function.x<-function(input){
  output<-input %>% 
    ...
    mutate(sizeOriginal=size) %>% #Here I am saving the original size data before log transforming it. I substract 1 because before I summed 1 to log transform it later
    #mutate(sizeNextOriginal=sizeNext-1) %>% 
     mutate(sizeNextOriginal=sizeNext) %>% 
...
```{r}
###ADDED THIS CHUNK TO KNOW THE SIZE OF INDIVIDUALS THAT HAD 1 SHOOT
##GROWTH AND SURVIVAL
#Copy data base with list
aggregate.height.for.seedlings<-data.all.years
...
#Apply function to list
aggregate.height.for.seedlings<-lapply(aggregate.height.for.seedlings,function.x)


#Make a list where I merge the 2013 with 2015 data and 2015 with 2017 data for estimating growth and survival between years
growth.survival.data.for.seedlings<-list(first.transition=merge(aggregate.height.for.seedlings[[1]],aggregate.height.for.seedlings[[2]],by=c("Patch","Plot","Clump"),all=T),second.transition=merge(aggregate.height.for.seedlings[[2]],aggregate.height.for.seedlings[[3]],by=c("Patch","Plot","Clump"),all=T))
...
for (i in 1:2){
  #growth.survival.data[[i]]$size.x<-growth.survival.data[[i]]$size.x+1
  growth.survival.data.for.seedlings[[i]]$height.x<-growth.survival.data.for.seedlings[[i]]$height.x
  colnames(growth.survival.data.for.seedlings[[i]])[5]<-"size"
  
  growth.survival.data.for.seedlings[[i]]$shoots.x<-growth.survival.data.for.seedlings[[i]]$shoots.x
  colnames(growth.survival.data.for.seedlings[[i]])[4]<-"shoots"
}

for (i in 1:2){
  #growth.survival.data[[i]]$size.y<-growth.survival.data[[i]]$size.y+1
  growth.survival.data.for.seedlings[[i]]$height.y<-growth.survival.data.for.seedlings[[i]]$height.y
  colnames(growth.survival.data.for.seedlings[[i]])[7]<-"sizeNext"
  
  growth.survival.data.for.seedlings[[i]]$shoots.y<-growth.survival.data.for.seedlings[[i]]$shoots.y
  colnames(growth.survival.data.for.seedlings[[i]])[6]<-"shootsNext"
}

#Add year of demographic vital rate
growth.survival.data.for.seedlings[[1]]$year<-as.factor(2015)
growth.survival.data.for.seedlings[[2]]$year<-as.factor(2017)
...
#Apply function
growth.survival.data.for.seedlings<-lapply(growth.survival.data.for.seedlings,function.x)

#Clean data base
growth.survival.data.for.seedlings<-bind_rows(growth.survival.data.for.seedlings[[1]],growth.survival.data.for.seedlings[[2]]) 

table(growth.survival.data.for.seedlings$shoots,growth.survival.data.for.seedlings$shootsNext)


growth.survival.data.for.seedlings %>% 
  filter(shoots==1 & shootsNext==1) %>% 
...
boxplot(seedling.size$Height_cm)

seedling.size %>% 
  filter(Height_cm>=15) %>% 
  ggplot(aes(y=Height_cm)) +
  geom_boxplot()
  

summary(seedling.size$Height_cm)
seedling.size %>% 
  filter(Height_cm>=15) %>% 
  select(Height_cm) %>% 
  summary()
...
summary(mod.1<-glm(sizeNext ~ size +  logpatch_size+percent_forest+elevation+mean_openess+year, family="poisson", data=data_growth_surv_diff_canopy))


library("lme4")

#GLMM nesting Patch
summary(mod.2<-glmer(sizeNext~scale(size) + scale(logpatch_size) + scale(percent_forest) + scale(elevation) + scale(mean_openess) + year+(1 | Patch),data=data_growth_surv_diff_canopy,family="poisson")) #Convergence error


anova(mod.1,mod.2)

#Without canopy openess
summary(mod.3<-update(mod.1,.~.-mean_openess))

#Without patch size
summary(mod.4<-update(mod.3,.~.-logpatch_size))

#Without percent of forest
summary(mod.5<-update(mod.4,.~.-percent_forest))

#Without elevation
summary(mod.6<-update(mod.5,.~.-elevation))

#Without year
summary(mod.7<-update(mod.6,.~.-year))


library("MuMIn")
Weights(AIC(mod.4,mod.5,mod.6,mod.7))
...
residuals<-resid(model.selected)
...
for (i in 2:length(demographic.output)){
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    demographic.output[[i]][[k]]$p.vec<-data.frame(cbind(coef(model.selected)[1],coef(model.selected)[2],sd(resid(model.selected))))
    
    colnames(demographic.output[[i]][[k]]$p.vec)<-c("growth.int","growth.size","growth.sd")
...
d = dpois(xp, mean=params$growth.int + params$growth.size * x, sd=params$growth.sd)
return(d)
}
...
u=exp(params$growth.int+params$growth.size*x)
return(u)
}
```
...
xx=seq(min(data_growth_surv_diff_canopy$size,data_growth_surv_diff_canopy$size,na.rm=T),max(data_growth_surv_diff_canopy$size,data_growth_surv_diff_canopy$sizeNext,na.rm=T),by=.01)
...
data_growth_surv_diff_canopy %>% ggplot(aes(x=size,y=sizeNext)) + geom_point(position = position_jitter(height = 0, width = 0.1), alpha = 0.5) + geom_smooth(method = "glm",se = F, 
        method.args = list(family = "poisson"),col="red") + geom_abline(intercept=0,slope=1,col="blue",linetype="dashed",size=1) +
...
#Plot pairs
pairs(data_surv_canopy[,c("surv","size","logpatch_size","percent_forest","elevation","mean_openess","year")],lower.panel=panel.cor,diag.panel=panel.hist,panel=panel.smooth)
...
#This model has the lowest AIC. Take out mean openess
summary(mod.3<-update(mod.2,.~. -scale(mean_openess)))
...
library("MuMIn")
Weights(AIC(mod.3,mod.4,mod.5)) #Last model is the best one


#Select the  best model
model.selected<-mod.5
...
for (i in 2:2){
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    demographic.output[[i]][[k]]$survival<-subset(coef(model.selected)$Patch %>% select(-year2017),as.numeric(substr(row.names(coef(model.selected)$Patch %>% select(-year2017)),1,2))==demographic.output[[i]][[k]]$data$Patch[1])
    #names(demographic.output[[i]][[k]]$survival)<-c("surv.int","surv.scale.size")
    names(demographic.output[[i]][[k]]$survival)<-c("surv.int","surv.elevation","surv.size") #Here the vector i going to differ in length compared to the vector in 2017 since I do not have a column dor year 2015 since is the group that is being compared in the regression as dummy variable
...
for (i in 3:3){
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    names(demographic.output[[i]][[k]]$survival)<-c("surv.int","surv.elevation","surv.size","surv.year.2017")#Here the vector i going to differ in length compared to the vector in 2017 since I do not have a column dor year 2015 since is the group that is being compared in the regression as dummy variable
    demographic.output[[i]][[k]]$p.vec<-cbind(demographic.output[[i]][[k]]$p.vec,demographic.output[[i]][[k]]$survival)
...
#Generate a sequence of sizes that go from the minimum to the maximum found in the data base
xx=seq(min(data_surv_canopy$size,data_surv_canopy$size,na.rm=T),max(data_surv_canopy$size,data_surv_canopy$sizeNext,na.rm=T),by=.01)
...
#Without patch size
summary(mod.3<-update(mod.1,.~. -logpatch_size))
...
#Withuot percent of forest
summary(mod.6<-update(mod.5,.~. -percent_forest))

#Without canopy openess
summary(mod.7<-update(mod.6,.~. -mean_openess))

#AIC selection
Weights(AIC(mod.2,mod.3,mod.4,mod.5,mod.6,mod.7))
...
#Check assumptions
par(nfrow=c(2,2))
plot(model.selected) #DO NOT KNOW IF ASSUMPTIONS ARE CORRECT
residuals.model<-resid(model.selected)
...
#Store first model
model.selected1<-model.selected
...
#Remove outlier (which was an individual with 40 shoots)
data_reproduction_analysis_without_fec2_outlier<-subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE") %>% 
  filter(size<40)
  
#Model without random structure
summary(mod.1<-glm(fec2 ~ size +  logpatch_size + percent_forest + elevation + mean_openess+year, family="poisson", data=data_reproduction_analysis_without_fec2_outlier))

library("lme4")

#GLMM nesting Patch
summary(mod.2<-glmer(fec2 ~ scale(size) +  scale(logpatch_size) + scale(percent_forest) + scale(elevation) + scale(mean_openess) + year+(1 | Patch),data=data_reproduction_analysis_without_fec2_outlier,family="poisson")) 

#GLMM nesting Patch and with random slope
#summay(mod.4<-glmer(fec2 ~ size +  logpatch_size + percent_forest + elevation + mean_openess + year + (fec2 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")) #Error convergence


#Without percent_forest
summary(mod.3<-update(mod.1,.~. -percent_forest))

#Without mean_openess
summary(mod.4<-update(mod.3,.~. -mean_openess))

#Withuot elevation
summary(mod.5<-update(mod.4,.~. -elevation))

#Withuot logpatch_size
...
model.selected2<-model.selected
...
model.selected3<-model.selected


#Check coefficients of the 3 models
coef(model.selected1)
coef(model.selected2)
coef(model.selected3)


#PREDICT FOR FIRST MODEL WITH ALL THE PLANTS
par(mfrow=c(1,3))
plot(data_reproduction_analysis_without_fec2_outlier$size,jitter(data_reproduction_analysis_without_fec2_outlier$fec2),xlab="Number of shoots",ylab="Number of inflorescences per clump",main="Number of inflorescences by size original",cex.axis=2,cex.lab=2,cex=2,cex.main=2,xlim=c(0,40))


#Generate a sequence of sizes so that I can predict values
#prediction<-seq(from=min(subset(data_reproduction_analysis,data_reproduction_analysis$year!=2017)$size),to=max(subset(data_reproduction_analysis,data_reproduction_analysis$year!=2017)$size),by=0.01)
prediction<-seq(from=min(data_reproduction_analysis_without_fec2_outlier$size),to=40,by=0.01)

#Make a data frame with the coefficents of the model
parameters<-coef(model.selected1)


res.pred.2013<-exp(parameters[1]+parameters[2]*prediction)
...
#PREDICT FOR SECOND MODEL WITH PLANTS WITH LESS THAN 40 SHOOTS
plot(data_reproduction_analysis_without_fec2_outlier$size,jitter(data_reproduction_analysis_without_fec2_outlier$fec2),xlab="Number of shoots",ylab="Number of inflorescences per clump",main="Number of inflorescences by size <40",cex.axis=2,cex.lab=2,cex=2,cex.main=2,xlim=c(0,40))
...
#Make a data frame with the coefficents of the model
parameters<-coef(model.selected2)
...
#PREDICT FOR SECOND MODEL WITH PLANTS WITH LESS THAN 150 SHOOTS
plot(data_reproduction_analysis_without_fec2_outlier$size,jitter(data_reproduction_analysis_without_fec2_outlier$fec2),xlab="Number of shoots",ylab="Number of inflorescences per clump",main="Number of inflorescences by size <15",cex.axis=2,cex.lab=2,cex=2,cex.main=2,xlim=c(0,40))
...
#Make a data frame with the coefficents of the model
parameters<-coef(model.selected3)
...
#I THINK THE BEST MODEL IS 2 (WITHOUT PLANTS LESS THAN 40 SHOOTS)
...
for (i in 2:length(demographic.output)){
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    demographic.output[[i]][[k]]$num.inflor.model<-coef(model.selected2)
...
Weights(AIC(mod.1,mod.3,mod.4,mod.5,mod.6)) #Best model is 6
...
for (i in 2:length(demographic.output)){
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    demographic.output[[i]][[k]]$num.bracts.model<-data.frame(mean.num.bract.per.infl.int=coef(model.selected)[1],mean.num.bract.per.infl.size=coef(model.selected)[2],mean.num.bract.per.infl.logpatch_size=coef(model.selected)[3])
...
p1<-data_reproduction_analysis %>% 
  ...
  geom_smooth(method="glm",col="red",method.args = list(family = "poisson"),se=FALSE) +
...
for (i in 2:3){
  for (k in 1:length(demographic.output[[i]])){
    ...
    #****ASK ADAM ABOUT THIS DATA. Seeds per fruit. The mean would be 2 because it can produce between 1 and 3 fruits. MODIFIED THIS TO DESACTIVATE IT
    ...
    #Here I have two trays per plot, so I will obtain the mean of the proportion of seeds that germinated at each plot. i DO NOT KNOW HERE IF I SHOULD PUT THE MEAN OF ALL THE PROPROTION OF TERMINATION OF ALL PLOTS SINCE I DID NOT FIND ANY ENVIRONMENTAL VARIABLE THAT COULD EXPLAIN PROPORTION OF SEED GERMINATED
...
```{r}
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    c$seedling.surv.mean.openness<-coef(regression.sel.surv.seedlings)[3]
  }
}
...
###ADD A FUNCTION FOR NUMBER OF SEEDS PER FLOWER (FRUIT NO SINCE WE KNOW)
```
