...
rm(aggregate.height,first_year,second_year,third_year,aggregate.prob.rep.num.infl,aggregate.num.bracts)
...
patches.info$logpatch_size<-log(patches.info$patch_size)
...
#for (i in 2:length(demographic.output)){
#  for (k in 1:nrow(patches.info)){

    #demographic.output[[i]][[k]]$data_growth_surv_diff_canopy %>% 
      #mutate(diff=demographic.output[[i]][[k]]$data_growth_surv_diff_canopy$sizeNext-demographic.output[[i]][[k]]$data_growth_surv_diff_canopy$size) %>% 
      #filter(!is.na(diff)) %>% 
      #inner_join(agg.openess,by="Patch")
  #}
  #}
...
#ANALYSIS OF GROWTH USING SIZET AND SIZET+1
#Plot pairs
pairs(data_growth_surv_diff_canopy[,c("sizeNext","size","logpatch_size","percent_forest","elevation","mean_openess")],lower.panel=panel.cor,diag.panel=panel.hist,panel=panel.smooth) #It would be great to add colors depending on the year


#Model without random structure
#mod.1<-gls(sizeNext ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(mean_openess)+year,method = "REML", data = data_growth_surv_diff_canopy)
...
#GLMM nesting plot in patch. THIS MODEL IS NOT RUNNING, SO I WILL USE MODEL 2
#mod.3<-lme(sizeNext~scale(size)+scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year,random = ~1 + sizeNext | Patch/Plot,data=data_growth_surv_diff_canopy,method="REML")
#summary(mod.3)

#The model with the lowest AIC is the one that includes patch/plot. I will use that for choosing a fixed part
#GLMM without year
#mod.4<-lme(sizeNext~scale(size)+scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(mean_openess),random = ~1  | Patch,data=data_growth_surv_diff_canopy,method="REML")
mod.4<-lme(sizeNext~size+logpatch_size+percent_forest+elevation+mean_openess,random = ~1 | Patch,data=data_growth_surv_diff_canopy,method="REML")
summary(mod.4)

#GLMM without patch size
#mod.5<-lme(sizeNext~scale(size)+scale(percent_forest)+scale(elevation)+scale(mean_openess),random = ~1  | Patch,data=data_growth_surv_diff_canopy,method="REML")
mod.5<-lme(sizeNext~size+percent_forest+elevation+mean_openess,random = ~1 | Patch,data=data_growth_surv_diff_canopy,method="REML")
summary(mod.5)

#GLMM without canopy openess
#mod.6<-lme(sizeNext~scale(size)+scale(percent_forest)+scale(elevation),random = ~1  | Patch,data=data_growth_surv_diff_canopy,method="REML")
mod.6<-lme(sizeNext~size+percent_forest+elevation,random = ~1 | Patch,data=data_growth_surv_diff_canopy,method="REML")
summary(mod.6)

#GLMM without percent of forest
#mod.7<-lme(sizeNext~scale(size)+scale(elevation),random = ~1  | Patch,data=data_growth_surv_diff_canopy,method="REML")
mod.7<-lme(sizeNext~size+elevation,random = ~1 | Patch,data=data_growth_surv_diff_canopy,method="REML")
summary(mod.7)

#GLMM without elevation
#mod.8<-lme(sizeNext~scale(size),random = ~1  | Patch,data=data_growth_surv_diff_canopy,method="REML")
mod.8<-lme(sizeNext~size,random = ~1 | Patch,data=data_growth_surv_diff_canopy,method="REML")
summary(mod.8)
...
#Model without random structure
#mod.1<- glm(surv ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year, data = data_surv_canopy, family = binomial) #I do not know if this is ok since I am not using REML
mod.1<- glm(surv ~ size +  logpatch_size+percent_forest+elevation+mean_openess+year, data = data_surv_canopy, family = binomial) #I do not know if this is ok since I am not using REML

#mod.1<-gls(surv ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year,method = "REML", data = data_surv_canopy, family = binomial)
summary(mod.1)
...
#mod.2 <- glmer(surv~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 | Patch),data=data_surv_canopy, family = binomial)
mod.2 <- glmer(surv~logpatch_size+percent_forest+elevation+mean_openess+size+ year + (1 | Patch),data=data_surv_canopy, family = binomial)#I have the following error Model failed to converge with max|grad| = 0.0015969 (tol = 0.001, component 1)
...
#GLMM nesting plot in patch but with random intercept only. I AM NOT USING REML, I DO NOT KNOW IF THAT IS CORRECT. 
#mod.3 <- glmer(surv~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial)
mod.3 <- glmer(surv~logpatch_size+percent_forest+elevation+mean_openess+size+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial) #Same erro as above

#mod.3<-lme(surv~scale(size)+scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year,random = ~1 + surv | Patch/Plot,data=data_surv_canopy,method="REML", family = binomial)
summary(mod.3)

#GLMM of of randome effects with patch but using random slope and intercept. I AM NOT USING REML, I DO NOT KNOW IF THAT IS CORRECT. 
#mod.4 <- glmer(surv~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 + surv | Patch),data=data_surv_canopy, family = binomial)
#summary(mod.4)

#GLMM of of randome effects with plot nested in patch but using random slope and intercept. I AM NOT USING REML, I DO NOT KNOW IF THAT IS CORRECT. 
#mod.5 <- glmer(surv~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 + surv | Patch/Plot),data=data_surv_canopy, family = binomial)
#summary(mod.5)


#I WILL CONTINUE WITH MODEL WITH RANDOM INTERCEPT WITH PLOT NESTED IN PATCH. HOWEVER, THE MODELS WITH RANDOM INTERCEPTS AND SLOPES HAVE A LOWER AIC BUT THERE IS AN ERROR THAT I DO NOT KNOW WHAT DOES IT MEAN
#I will take out canopy openess
#mod.6 <- glmer(surv~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(size)+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial)
mod.6 <- glmer(surv~logpatch_size+percent_forest+elevation+size+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial) #I have convergence error
summary(mod.6)

#I will take out patch size
#mod.7 <- glmer(surv~scale(percent_forest)+scale(elevation)+scale(size)+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial)
mod.7 <- glmer(surv~percent_forest+elevation+size+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial) #I have convergence error
summary(mod.7)

#I will take out percent of forest
#mod.8 <- glmer(surv~scale(elevation)+scale(size)+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial)
mod.8 <- glmer(surv~elevation+size+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial) #I have convergence error
summary(mod.8)

#Elevation is marginally 
#mod.9 <- glmer(surv~scale(size)+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial)
mod.9 <- glmer(surv~size+ year + (1 | Patch/Plot),data=data_surv_canopy, family = binomial) #I have convergence error
summary(mod.9)

library("MuMIn")
Weights(AIC(mod.6,mod.7,mod.8,mod.9))
...
###*****I WILL USE THE FOLLOWING MODEL SINCE I DO NOT KNOW HOW TO EXTRACT THE INFORMATION FORM A MODEL THAT HAS A RANDOM FACTOR PATCH/PLOT, SO I WILL USE A MODEL WITH RANDOM FACTOR AS PLOT ONLY
mod.6 <- glmer(surv~size+ year + 
                 (1 | Patch),
               data=data_surv_canopy, 
               family = binomial)
summary(mod.6)
...
#print(xyplot(surv~size|year,data=data_surv_canopy,main= "Survival")+layer(panel.smoother(..., col = "red")))

#print(xyplot(surv~sizeOriginal|year,data=data_surv_canopy,main= "Survival")+layer(panel.smoother(..., col = "red")))
...
#Based on AIC, model 8 is the best one.

#jpeg("Graphs/Clump/survival.jpg",quality=100,width=1000,height=700)
#par(mar=c(5,6,5,2))
#plot(data_surv_canopy$size,data_surv_canopy$surv,xlab="Log plant height",ylab="Probability of survival",cex.axis=3,cex.lab=3,cex=3,cex.main=3,main="Survival")

#par(new=T)
#Plot regression line
#abline(mod.6$coefficients$fixed[1:2],col="red",lwd=3)
#dev.off()
...
#Plot
plot(subset(data_surv_canopy,data_surv_canopy$year==2015)$size,jitter(subset(data_surv_canopy,data_surv_canopy$year==2015)$surv),xlab="Size (Log height (cm))",ylab="Probability of survival",main="Year 2013-2015")
lines(xx,predicted.values.2015, col="red",lwd=3)
abline(h=1,col="yellow",lty=2,lwd=2)
...
#Plot
plot(subset(data_surv_canopy,data_surv_canopy$year==2017)$size,jitter(subset(data_surv_canopy,data_surv_canopy$year==2017)$surv),xlab="Size (Log height (cm))",ylab="Probability of survival",main="Year 2015-2017")
...
all.patches<-rbind(all.patches,all.patches,all.patches)

#Assigning plot number to each patch
all.patches$Plot<-rep(1:3,each=20)
...
```{r,echo=F}
#Plot pairs
pairs(density.reproductive[,c("density","logpatch_size","percent_forest","elevation","mean_openess","year")],lower.panel=panel.cor,diag.panel=panel.hist,panel=panel.smooth) #It would be great to add colors depending on the year

#Transform variable. IT IS NOT NORMALLY DISTRIBUTED BUT I WILL CONTINUE WITH THE TRANSFORMED DATA
density.reproductive$log.density<-log(density.reproductive$density+1)

#Plot
pairs(density.reproductive[,c("log.density","logpatch_size","percent_forest","elevation","mean_openess","year")],lower.panel=panel.cor,diag.panel=panel.hist,panel=panel.smooth)

#Model without random structure
mod.1<-gls(log.density ~ logpatch_size + percent_forest + elevation + mean_openess + year,method = "REML", data = density.reproductive)
summary(mod.1)


#GLMM nesting patch
mod.2<-lme(log.density ~ logpatch_size + percent_forest + elevation + mean_openess + year,random = ~1 | Patch,data=density.reproductive,method="REML")
summary(mod.2)

#Without patch size
mod.3<-lme(log.density ~ percent_forest + elevation + mean_openess + year,random = ~1 | Patch,data=density.reproductive,method="REML")
summary(mod.3)

#Without percent forest
mod.4<-lme(log.density ~ elevation + mean_openess + year,random = ~1 | Patch,data=density.reproductive,method="REML")
summary(mod.4)

#Without elevation
mod.5<-lme(log.density ~ mean_openess + year,random = ~1 | Patch,data=density.reproductive,method="REML")
summary(mod.5)

#Without canopy openess
mod.6<-lme(log.density ~ year,random = ~1 | Patch,data=density.reproductive,method="REML")
summary(mod.6)

#WITH PLOT 10X10 ACTIVE, ELEVATION IS MARGINALLY SIGNIFICANT P=0.07 BUT THERE ARE STILL DIFFERENCES BETWEEN YEARS

jpeg("Graphs/Clump/Density reproductive individuals.jpg",quality=100,width=1000,height=700)
#Plot with line
par(mar=c(6,6,3,2))
plot(density.reproductive$year,density.reproductive$log.density,xlab="Year",ylab="Log density (ind/m2)",main="Density of reproductive individuals",cex.lab=2,cex.axis=2,cex.main=2,cex=2)
...
density.reproductive %>% 
  mutate(year=as.factor(year)) %>% 
  ggplot(aes(x=Patch,y=density,fill=year)) +
  geom_bar(stat="identity", position=position_dodge(),width=0.5)+
    scale_fill_grey() +
  ylab("Density of reproductive individuals (ind/m2)")
ggsave("Graphs/Clump/Density reproductive individuals by patch and year barplot.jpg")
...
```{r,echo=F}
#Plot pairs
pairs(density[,c("density","logpatch_size","percent_forest","elevation","mean_openess","year")],lower.panel=panel.cor,diag.panel=panel.hist,panel=panel.smooth) #It would be great to add colors depending on the year

#Log transform density
density$log.density<-log(density$density)

pairs(density[,c("log.density","logpatch_size","percent_forest","elevation","mean_openess","year")],lower.panel=panel.cor,diag.panel=panel.hist,panel=panel.smooth) #It would be great to add colors depending on the year

#Model without random structure
mod.1<-gls(log.density ~ logpatch_size + percent_forest + elevation + mean_openess + year,method = "REML", data = density)
summary(mod.1)


#GLMM nesting patch
mod.2<-lme(log.density ~ logpatch_size + percent_forest + elevation + mean_openess + year,random = ~1 | Patch,data=density,method="REML")
summary(mod.2)

#Without mean openess
mod.3<-lme(log.density ~ logpatch_size + percent_forest + elevation + year,random = ~1 | Patch,data=density,method="REML")
summary(mod.3)

#Without percent forest
mod.4<-lme(log.density ~ logpatch_size + elevation + year,random = ~1 | Patch,data=density,method="REML")
summary(mod.4)

#Without patch size
mod.5<-lme(log.density ~ elevation + year,random = ~1 | Patch,data=density,method="REML")
summary(mod.5)


p1<-density %>%
  ggplot(aes(x=elevation,y=log.density)) + 
  geom_point() +
  geom_smooth(method="lm",col="red") +
  xlab("Elevation (m.a.s.l.)") +
  ylab("Log density (ind/m2)") +
  geom_text(x=1510, y=0.3, label="a)",fontface="bold")
ggsave("Graphs/Clump/Density all individuals vs elevation.jpg")

p2<-density %>%
  ggplot(aes(x=year,y=log.density)) +
  geom_boxplot() +
  xlab("Year") +
  ylab("Log density (ind/m2)") +
  geom_text(x=2.5, y=0.3, label="b)",fontface="bold")
ggsave("Graphs/Clump/Density all individuals vs year.jpg")

library("gridExtra")
p3<-grid.arrange(p1, p2, ncol=2)
ggsave("Graphs/Clump/Density all individuals vs elevation and year.jpg",plot=p3)
...
density.patch %>% 
  ...
  geom_bar(stat="identity", position=position_dodge(),width=0.5)+
    scale_fill_grey() +
  ylab("Density of individuals (ind/m2)")
ggsave("Graphs/Clump/Density individuals by patch and year barplot.jpg")
...
###############################
###FECUNDITY
#Probability of reproduction (fec1)
...
#Model without random structure
#mod.1<- glm(fec1 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year, data = data_reproduction_analysis, family = binomial) #I do not know if this is ok since I am not using REML
mod.1<- glm(fec1 ~ size +  logpatch_size + percent_forest + elevation + mean_openess+year, data = data_reproduction_analysis, family = binomial) #I do not know if this is ok since I am not using REML
summary(mod.1)

#****I WAS READING HOW TO DO A glmm IN THE BOOK
#GLMM with random effects of patch but with random intercept only.  I AM NOT USING REML, I DO NOT KNOW IF THAT IS CORRECT. 
#mod.2 <- glmer(fec1~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 | Patch),data=data_reproduction_analysis, family = binomial)
mod.2 <- glmer(fec1~logpatch_size+percent_forest+elevation+mean_openess+size+ year + (1 | Patch),data=data_reproduction_analysis, family = binomial) #Convergence error
summary(mod.2)

#GLMM nesting plot in patch but with random intercept only. I AM NOT USING REML, I DO NOT KNOW IF THAT IS CORRECT. 
#mod.3 <- glmer(fec1~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 | Patch/Plot),data=data_reproduction_analysis, family = binomial)
mod.3 <- glmer(fec1~logpatch_size+percent_forest+elevation+mean_openess+size+ year + (1 | Patch/Plot),data=data_reproduction_analysis, family = binomial) #Convergence error
summary(mod.3)

#GLMM of of randome effects with patch but using random slope and intercept. I AM NOT USING REML, I DO NOT KNOW IF THAT IS CORRECT. 
#mod.4 <- glmer(fec1~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 + fec1 | Patch),data=data_reproduction_analysis, family = binomial)
#summary(mod.4)

#GLMM of of randome effects with plot nested in patch but using random slope and intercept. I AM NOT USING REML, I DO NOT KNOW IF THAT IS CORRECT. 
#mod.5 <- glmer(fec1~scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 + fec1 | Patch/Plot),data=data_reproduction_analysis, family = binomial)
#summary(mod.5)

#I WILL USE MODEL 2 SINCE I DO NOT KNOW HOW TO INCLUDE IN THE IPM OTHER KIND OF MODELS
#Without percent forest
#mod.6 <- glmer(fec1~scale(logpatch_size)+scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 | Patch),data=data_reproduction_analysis, family = binomial)
mod.6 <- glmer(fec1~logpatch_size+elevation+mean_openess+size+ year + (1 | Patch),data=data_reproduction_analysis, family = binomial) #Error model convergence
summary(mod.6) 

#Without patch size
#mod.7 <- glmer(fec1~scale(elevation)+scale(100-mean_openess)+scale(size)+ year + (1 | Patch),data=data_reproduction_analysis, family = binomial)
mod.7 <- glmer(fec1~elevation+mean_openess+size+ year + (1 | Patch),data=data_reproduction_analysis, family = binomial) #Error model convergence
summary(mod.7)

#Without elevation (although elevation is marginally significant)
#mod.8 <- glmer(fec1~scale(100-mean_openess)+scale(size)+ year + (1 | Patch),data=data_reproduction_analysis, family = binomial)
mod.8 <- glmer(fec1~mean_openess+size+ year + (1 | Patch),data=data_reproduction_analysis, family = binomial) #Error model convergence
summary(mod.8)

#AIC selection
Weights(AIC(mod.6,mod.7,mod.8))

#Selected mod.7 accorrding to AIC
model.selected<-mod.7 #WITH 10X10M ACTIVE ELEVATION, MEAN OPENNES, SIZE AND YEAR ARE STATISTICALLY SIGNIFICANT
```
...
for (i in 2:2){
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    #names(demographic.output[[i]][[k]]$prob.repr.mod)<-c("prob.rep.int","prob.rep.scale.size","prob.rep.scale.elevation","prob.rep.scale.100.minus.mean_openess")
...
#Here I add the information of the parameters of 2015
#for (i in 2:2){
  
#  for (k in 1:length(demographic.output[[i]])){
#    demographic.output[[i]][[k]]$p.vec<-demographic.output[[1]][[k]]$p.vec
    
#demographic.output[[i]][[k]]$p.vec$pr.2015<-demographic.output[[i]][[k]]$prob.repr.mod[1,ncol(demographic.output[[i]][[k]]$prob.repr.mod)]
...
#    demographic.output[[i]][[k]]$prob.repr.mod<-subset(coef(model.selected)$Patch[,c(1:4,6)],as.numeric(substr(row.names(coef(model.selected)$Patch),1,2))==demographic.output[[i]][[k]]$data$Patch[1])
...
jpeg("Graphs/Clump/probability of reproduction vs plant height by year.jpg",quality=100,width=600,height=600)
par(mar=c(5,6,5,6))
#size
plot(data_reproduction_analysis$size,jitter(data_reproduction_analysis$fec1),xlab="Log height",ylab="Probability of reproduction",main="Probability of reproduction by size",cex.axis=2,cex.lab=2,cex=2,cex.main=2)
...
#Predict values
res.pred.2013<-exp(mean(parameters$`(Intercept)`)+(unique(parameters$size)*prediction)+unique(parameters$elevation)+unique(parameters$mean_openess))

res.pred.2015<-exp(mean(parameters$`(Intercept)`)+(unique(parameters$size)*prediction)+unique(parameters$elevation)+unique(parameters$mean_openess)+unique(parameters$year2015))

res.pred.2017<-exp(mean(parameters$`(Intercept)`)+(unique(parameters$size)*prediction)+unique(parameters$elevation)+unique(parameters$mean_openess)+unique(parameters$year2017))
...
#Put legend
legend("topleft",legend=c("2013","2015","2017"),col=c("red","green","orange"),bty="n",lty=1,lwd=2,cex=1.5)
...
data_reproduction_analysis %>% 
  ggplot(aes(x=size,y=fec1),group=year) + 
  geom_point() + 
  geom_smooth(method = "glm", 
    method.args = list(family = "binomial"), 
    se = TRUE,
...
p0<-data_reproduction_analysis %>% 
  ...
  ylab("Probability of reproduction") +
  labs(colour = 'Year')  +
  geom_text(x=8.7, y=1.1, label="a)") +
  theme(legend.position="top",
        legend.justification ="center") # +
  #scale_colour_discrete(name = "Year", labels = c("A", "B", "C")) + theme_bw()
ggsave("Graphs/Clump/Probability of reproduction by year shoots.jpg")

data_reproduction_analysis %>% 
  ggplot(aes(x=size.original,y=fec1)) + 
  geom_point(position = position_jitter(height = 0.1, width = 0.1), alpha = 0.5) + 
  geom_smooth(method = "glm", 
    method.args = list(family = "binomial"), 
    se = TRUE,
    aes(colour = year)) +
  xlab("Number of shoots") +
  ylab("Probability of reproduction") +
  labs(colour = 'Year')  +
  geom_text(x=8.7, y=1.1, label="a)") +
  theme(legend.position="top",
        legend.justification ="center") # +
  #scale_colour_discrete(name = "Year", labels = c("A", "B", "C")) + theme_bw()
ggsave("Graphs/Clump/Probability of reproduction by year shoots without log.jpg")
...
library("gridExtra")
p3<-grid.arrange(p0,p1, p2,ncol=3)
...
#Model without random structure
#mod.1<-glm(fec2 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year, family="poisson", data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"))
mod.1<-glm(fec2 ~ size +  logpatch_size + percent_forest + elevation + mean_openess+year, family="poisson", data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"))
...
#GLMM nesting Patch
#mod.2<-glmer(fec2 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.2<-glmer(fec2 ~ size +  logpatch_size + percent_forest + elevation + mean_openess+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson") #Error convergence
summary(mod.2)

#GLMM nesting Plot in Patch
#mod.3<-glmer(fec2 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(1 | Patch/Plot),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.3<-glmer(fec2 ~ size +  logpatch_size + percent_forest + elevation + mean_openess+year+(1 | Patch/Plot),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson") #Error convergence

summary(mod.3)

#GLMM nesting Patch and with random slope
#mod.4<-glmer(fec2 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(fec2 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.4<-glmer(fec2 ~ size +  logpatch_size + percent_forest + elevation + mean_openess + year + (fec2 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson") #Error convergence

summary(mod.4)

#GLMM nesting Patch
#mod.5<-glmer(fec2 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(fec2 | Patch/Plot),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.5<-glmer(fec2 ~ size +  logpatch_size + percent_forest + elevation + mean_openess + year + (fec2 | Patch/Plot),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson") #error convergence

summary(mod.5)



#The best model includes a random slope and intercept and the random effects are patch/plot. I will not use it for now since I do not know how to put it in my model
Weights(AIC(mod.1,mod.2,mod.3,mod.4,mod.5))

#The best model is without a random structure, but I will not use it since I think it should include a random structure because of the violation of the independence of the data
Weights(AIC(mod.1,mod.2,mod.3))

#From these models, the best one is model 2, so I will use it
Weights(AIC(mod.2,mod.3))

#Without patch size
#mod.6<-glmer(fec2 ~ scale(size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.6<-glmer(fec2 ~ size + percent_forest + elevation + mean_openess+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson") #Error convergence
summary(mod.6)

#Without year
#mod.7<-glmer(fec2 ~ scale(size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.7<-glmer(fec2 ~ size + percent_forest + elevation + mean_openess+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson") #Error convergence
summary(mod.7)

#Withuot elevation
#mod.8<-glmer(fec2 ~ scale(size)+scale(elevation)+scale(100-mean_openess)+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.8<-glmer(fec2 ~ size + percent_forest  + mean_openess+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson") #Error convergence
summary(mod.8)

#Without mean_openess
#mod.9<-glmer(fec2 ~ scale(size)+scale(100-mean_openess)+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.9<-glmer(fec2 ~ size + percent_forest +(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson") #Error convergence
summary(mod.9)

#Without percent forest
#mod.10<-glmer(fec2 ~ scale(size)+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
mod.10<-glmer(fec2 ~ size +(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),family="poisson")
summary(mod.10)

mod.11<-nls(log(fec2) ~ I(exp(1)^(a + b * size)), data =subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"),start = list(a = 0, b = 1), trace = T)
summary(mod.11)

#I tired to log transform the response variable and run a liner model but id did not work very well. 
#mod.12<-lm(log(fec2)~size,data =subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec2)=="FALSE"))
#summary(mod.12)

#For now I will select this model
model.selected<-mod.11
#AIC selection
Weights(AIC(mod.6,mod.7,mod.8,mod.9,mod.10,mod.11))

#Select best model
#model.selected<-mod.9
#model.selected<-mod.11
```
...
for (i in 2:length(demographic.output)){
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    demographic.output[[i]][[k]]$num.inflor.model<-coef(mod.11)
...
jpeg("Graphs/Clump/number of inflorescences.jpg",quality=100,width=1000,height=700)
par(mar=c(5,6,5,2))
#size
plot(data_reproduction_analysis$size,data_reproduction_analysis$fec2,xlab="Log height",ylab="Number of inflorescences",cex.axis=2,cex.lab=2,cex=2,cex.main=2,main="Number of inflorescences")
...
res.pred<-exp(predict(mod.11,list(size=prediction)))
...
data_reproduction_analysis %>% 
  ggplot(aes(x=size,y=fec2)) +
  geom_point(position=position_jitter(height=0.3,width=0.1),alpha=0.5) +
  geom_smooth(method="lm",formula= (y ~ exp(x)),col="red") +
  xlab("Log of number of shoots") +
  ylab("Number of inflorescences")
ggsave("Graphs/New/Number of inflorescences shoots.jpg")

#
#plot(data_reproduction_analysis$percent_forest,data_reproduction_analysis$fec2,xlab="Percent of forest in a 1 km radius",ylab="Number of inflorescences",cex.axis=3,cex.lab=3,cex=3,cex.main=3,main="Number of inflorescences")
...
#Model without random structure
#mod.1<-glm(fec3 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year, family="poisson", data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"))
...
#GLMM nesting Patch
#mod.2<-glmer(fec3 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")

mod.2<-glmer(fec3 ~ size +  logpatch_size + percent_forest + elevation + mean_openess+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson") #Convergence error
summary(mod.2)

#GLMM nesting Plot in Patch
#mod.3<-glmer(fec3 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(1 | Patch/Plot),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")
mod.3<-glmer(fec3 ~ size +  logpatch_size + percent_forest + elevation + mean_openess + year + (1 | Patch/Plot),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson") #Convergence error

summary(mod.3)

###MODEL 4 AND 5 DO NOT WANT TO WORK
#GLMM nesting Patch and with random slope
#mod.4<-glmer(fec3 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(fec2 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")

#summary(mod.4)

#GLMM nesting Patch
#mod.5<-glmer(fec3 ~ scale(size) +  scale(logpatch_size)+scale(percent_forest)+scale(elevation)+scale(100-mean_openess)+year+(fec2 | Patch/Plot),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")

#summary(mod.5)



#The best model is the one without a random structure. I will run weights with the models that have a random structure. 
Weights(AIC(mod.1,mod.2,mod.3))

#The best model is the one with random intrecept only 
Weights(AIC(mod.2,mod.3))

#Without percent of forest
#mod.6<-glmer(fec3 ~ scale(size) +  scale(logpatch_size)+scale(elevation)+scale(100-mean_openess)+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")
mod.6<-glmer(fec3 ~ size +  logpatch_size + elevation + mean_openess+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson") #Error convergence
summary(mod.6)

#Without elevation
#mod.7<-glmer(fec3 ~ scale(size) +  scale(logpatch_size)+scale(100-mean_openess)+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")
mod.7<-glmer(fec3 ~ size +  logpatch_size + mean_openess+year+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")

summary(mod.7)

#Without year
#mod.8<-glmer(fec3 ~ scale(size) +  scale(logpatch_size)+scale(100-mean_openess)+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")
mod.8<-glmer(fec3 ~ size +  logpatch_size + mean_openess+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")

summary(mod.8)

#Without mean canopy openess
#mod.9<-glmer(fec3 ~ scale(size) +  scale(logpatch_size)+(1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")
mod.9<-glmer(fec3 ~ size +  logpatch_size + (1 | Patch),data=subset(data_reproduction_analysis,is.na(data_reproduction_analysis$fec3)=="FALSE"),family="poisson")

summary(mod.9)

Weights(AIC(mod.6,mod.7,mod.8,mod.9)) #Best model is 8
...
for (i in 2:length(demographic.output)){
  ...
  for (k in 1:length(demographic.output[[i]])){
    ...
    demographic.output[[i]][[k]]$num.bracts.model<-subset(coef(model.selected)$Patch,as.numeric(substr(row.names(coef(model.selected)$Patch),1,2))==demographic.output[[i]][[k]]$data$Patch[1])
    
    #names(demographic.output[[i]][[k]]$num.bracts.model)<-c("mean.num.bract.per.infl.int","mean.num.bract.per.infl.scale.size","mean.num.bract.per.infl.scale.logpatch_size")
